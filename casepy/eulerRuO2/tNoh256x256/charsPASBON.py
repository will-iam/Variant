...
def buildme(quantityDict, coords_to_uid, coords_to_bc):
    ...
    for k in range(1, BClayer + 1):
        ...
        for i in range(-k, Nx - 1 + k):
            ...
            y = (Ny + 0.5) * dy
            r = np.sqrt(x**2 + y**2) 
            coords_to_bc[(i, Ny - 1 + k)] = {'T': {"rho": u0 / r}, 'D': {"rhoE": 1/2*u0**2*rho0, "pressure": 0,"rhou_x": -u0*x/sqrt(x**2+y**2)*rho0, "rhou_y": -u0*y/sqrt(x**2+y**2)*rho0}}
        ...
        for j in range(Ny, -k, -1):
            ...
            y = (j + 0.5) * dy
            r = np.sqrt(x**2 + y**2) 
            coords_to_bc[(Nx - 1 + k, j)] = {'T': {"rho": u0 / r}, 'D': {"rhoE": 1/2*u0**2*rho0, "pressure": 0,"rhou_x": -u0*x/sqrt(x**2+y**2)*rho0, "rhou_y": -u0*y/sqrt(x**2+y**2)*rho0}}
