...
if parser.args.x:
    caseSize[0] = parser.args.x
if parser.args.y:
    caseSize[1] = parser.args.y
...
firstValueMaxComputeTime = sorted(maxComputeSumTimeDict.values())[0]
...
for k in loopTimeWoHTDict:
    ax.plot(np.full(len(loopTimeWoHTDict[k]), k), loopTimeWoHTDict[k], 'b+')
for k in loopTimeDict:
    ax.plot(np.full(len(loopTimeDict[k]), k), loopTimeDict[k], 'bo')
