...
parser.add_argument("--x", type = int, help = "X case size (in cell number)", required=False)
parser.add_argument("--y", type = int, help = "Y case size (in cell number)", required=False)
parser.add_argument("--res", type = int, help = "Resource used", required=False)
parser.add_argument("--nolog", action='store_true', help = "no log plot", required=False)
...
def makeKey(attrnames, data_line):
    ...
    if len(nIterationsList) == 0:
        print("Erreur dans le fichier de données : nombre de runs nul.")
        raise ValueError

    for n in nIterationsList:
        if n != nIterationsList[0]:
            print("Erreur dans le fichier de données : nombre de runs et résultats incohérents.")
            raise ValueError
    ...
    with open(filepath, 'r') as f:
        ...
        for line in f.readlines():
            ...
            for el in line.split(';'):
                if el[0] in ['(', '[']:
                    el = el[1:-1].replace(",", "").split()
                    try:
                        data_line.append([float(e) for e in el])
                    except ValueError:
                        data_line.append([str(e) for e in el])
                else:
                    try:
            ...
            if 'nSizeX' in filterDict.keys() and filterDict['nSizeX'] != nSizeX:
                #print("Filtered mismatch sizeX %s != %s" % (filterDict['nSizeX'], nSizeX))
                continue

            nSizeY = int(data_line[attrnames.index('nSizeY')])
            if 'nSizeY' in filterDict.keys() and filterDict['nSizeY'] != nSizeY:
                #print("Filtered mismatch sizeY %s != %s" % (filterDict['nSizeY'], nSizeY))
                continue
            ...
            sdsGeom = data_line[attrnames.index('SDSgeom')]
            ...
            ## Normale
            meanLT = float(loopTimeList[0])/normalizer
            stdLT = 0.
            if runNumber > 1:
                meanLT, stdLT = stats.norm.fit(loopTimeList)
                relDeviation = 100.0 * stdLT / meanLT
                if relDeviation > 8:
                    print "in", filepath,":"
                    print loopTimeList
                    print "Moyenne des temps : %s, écart-type : %s, en pourcentage de la moyenne : %s %%" % (meanLT, stdLT, relDeviation)
                    print "Don't accept these dirty data"
                    continue
            ...
            for i in range(0, runNumber):
                ...
                data[key].append({"point": point, "loopTime": loopTime, "minComputeSum": minComputeSum, "maxComputeSum": maxComputeSum, "maxIterationSum": maxIterationSum,
                         ...
                         "machine": machine, "nCommonSDS": nCommonSDS, "sdsGeom": sdsGeom, "meanLoopTime": meanLT/normalizer, "stdLoopTime": stdLT/normalizer, "runNumber": runNumber})
                #if loopTime < 15.e-6 and nCoresPerSDD == 64 and nSDD_X == 1 and nSDD_Y == 1 and extractKey(key)['Nt'] <= 4.5 and extractKey(key)['Nt'] > 4.35:
                #    print loopTime, filepath, loopTime * normalizer
