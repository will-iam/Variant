...
params['SDSgeom'] = ['line']
SDSsize = 256
nThreads_list = [64, 72, 80, 88, 96, 104, 112, 120, 128]       
#i = 64
#while i <= args.max_thread_number * 2:
#    nThreads_list.append(i)
#    i += 1
...
tmp_dir = os.path.join(config.tmp_dir, 'perfs_sds_thread')
io.make_sure_path_exists(tmp_dir)

print "Start finding SDS Optimal Number"
for nThreads in nThreads_list:
	...
	test['nSDS'] = compute_sds_number(case_path, test['nSDD'][0] * test['nSDD'][1], SDSsize)
        ...
        io.make_sure_path_exists(os.path.join(config.results_dir,
            'perfs_sds_thread', cn))
        results_path = os.path.join(config.results_dir, 'perfs_sds_thread', 'results_data.csv')
