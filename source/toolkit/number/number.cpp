...
bool Number::equal(real a, real b, real precision) {
    ...
    std::cerr << "Absolute error: " << Console::_red << std::scientific << std::setprecision(digitsize);
    std::cerr << vabs(a - b) << Console::_normal << " > ";
    std::cerr << precision << std::endl;

    std::cerr << "Relative error: " << Console::_red << std::scientific << std::setprecision(digitsize);
