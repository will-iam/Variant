...
params['nSDD'] = [(1, 1)]
params['SDSgeom'] = ['line']
SDSsize = [128, 256, 512, 1024, 2048, 8192]
params['nThreads'] = [2, 4, 16, 32]
...
tmp_dir = os.path.join(config.tmp_dir, 'perfs_sds_size')

#fig = plt.figure(0, figsize=(9, 6))
#ax = fig.add_subplot(111)

print "Start SDS Size Test"
...
for sds_size in SDSsize:
    ...
    io.make_sure_path_exists(os.path.join(config.results_dir,
        'perfs_sds_size', cn))
    results_path = os.path.join(config.results_dir, 'perfs_sds_size', cn, 'results_data.csv')
