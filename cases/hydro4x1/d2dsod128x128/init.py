...
for k in range(1, BClayer + 1):
    ...
    for j in range(-k, Ny - 1 + k):
        coords_to_bc[(-k, j)] = (BCtype, {"rho": 1.0, "rhoe" : 1.0 / (gamma - 1.0)})

    # Top border
    for i in range(-k, Nx - 1 + k):
        coords_to_bc[(i, Ny - 1 + k)] = (BCtype, {"rho": 0.125, "rhoe" : 0.1 / (gamma - 1.0)})

    # Right border
    for j in range(Ny, -k, -1):
        coords_to_bc[(Nx - 1 + k, j)] = (BCtype, {"rho": 0.125, "rhoe" : 0.1 / (gamma - 1.0)})

    # Bottom border
    for i in range(Nx, -k, -1):
        coords_to_bc[(i, -k)] = (BCtype, {"rho": 1.0, "rhoe" : 1.0 / (gamma - 1.0)})
