...
for i in range(Nx):
    for j in range(Ny):
        ...
        y = j * dy - ly / 2
        if x != 0 or y != 0:
            ux_uid_to_val[coords_to_uid[(i, j)]] = -y / (sqrt(x**2 + y**2))
            uy_uid_to_val[coords_to_uid[(i, j)]] = x / (sqrt(x**2 + y**2))
        else:
            ux_uid_to_val[coords_to_uid[(i, j)]] = 0#2 * (x - lx / 4)
            uy_uid_to_val[coords_to_uid[(i, j)]] = 0
