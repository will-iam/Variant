...
import test_cases.transport as tc
...
for i in range(Nx):
    for j in range(Ny):
        x = float(i) / Nx
        y = float(i) / Ny
        rho_uid_to_val[coords_to_uid[(i, j)]] = tc.rho_init(x, y)
...
for i in range(Nx):
    for j in range(Ny):
        x = i * dx - lx / 2
        y = j * dy - ly / 2
        ux_uid_to_val[coords_to_uid[(i, j)]] = tc.ux#2 * (x - lx / 4)#-y / (sqrt(x**2 + y**2))
        uy_uid_to_val[coords_to_uid[(i, j)]] = tc.uy#x / (sqrt(x**2 + y**2))
