...
#params['nThreads'] = []
#i = 0
#while 2**i <= args.max_thread_number:
#    params['nThreads'].append(2**i)
#    i += 2
...
tmp_dir = os.path.join(config.tmp_dir, 'perfs_sds_weakscal')
io.make_sure_path_exists(tmp_dir)

print "Start SDS Weak Scalability Test"
for p in SDSgeom_list:
    boxplots_data = []
    median_data = []
	...
	test['nSDS'] = compute_sds_number(case_path, test['nSDD'][0] * test['nSDD'][1], sds_size)
        ...
        # Init results dir
        io.make_sure_path_exists(os.path.join(config.results_dir, 'perfs_sds_weakscal', cn))
        results_path = os.path.join(config.results_dir, 'perfs_sds_weakscal', cn, 'results_data.csv')
